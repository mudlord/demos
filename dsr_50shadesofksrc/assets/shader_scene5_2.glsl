...
float roundbox2 = length(max(abs(p-vec3( -0.,sc1_cubeup, 2.0))-vec3(0.25),0.0)) - 0.01 + clamp(sin((p.x+p.y+p.z)*23.0)
     *0.02, 0.0, 1.0);   


  float d0 = length(max(abs(p) - 0.5, 0.0))- 0.01 + clamp(sin((p.x+p.y+p.z)*20.0)*0.03, 0.0, 1.0); 
  float d1 = length(p-vec3(0.0,0.5,0.0)) - 0.6; 	
	float cage = opS(d0,d1);
   p = rotatex(p, 0.6*time - 2.0);
  p = rotatez(p, 0.4*time - 2.0);
  ...
  float cageball = (cage<ball) ?cage : ball;
  ...
  float cross = opScale2(q-vec3( 0.0,sc1_mengercrossup, 0.0),0.1);
    vec2 res = opU( vec2( sdPlane(pos), 1.0 ),
	                vec2(menger , 2.0 ) );
 res = opU(res,vec2(cageball,2.0)); 
 res = opU(res,vec2(roundbox2,1.0));
    return res;
}
...
vec3 lights = (spec3*plasma(rd.xy)) + (spec2*plasma(ref.xy))+ (spec*plasma(ref.xy)) + (spec1*plasma(pos.xy)); 		
      ...
      if(m == 2.)
      {
       ...
       float occ = calcAO( pos, nor );
        ...
        dom *= softshadow( pos, ref, 0.02, 2.5 );
		    ...
		    vec3 brdf = vec3(0.0);
        brdf += 0.30*amb*plasma(ref.xy)*occ;

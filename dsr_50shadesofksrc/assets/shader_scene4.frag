...
float sdBox2( vec3 p, vec3 b )
{
  vec3 d = abs(p) - b;
  return min(max(d.x,max(d.y,d.z)),0.0) +
         length(max(d,0.1)) +clamp(sin((p.z*4.)*3.)*0.04, 0.0, 1.0);
}
...
vec2 map( in vec3 pos )
{
 ...
 float roundbox = length(max(abs(q-vec3( .0,-0.7, 0.0))-vec3(0.25),0.0)) - 0.02 + clamp(sin((q.x+q.y+q.z)*23.0)
     *0.02, 0.0, 1.0);

  //return length(max(abs(p)-b,0.0));
   
  float menger = opScale(q-vec3( 0.0,-0.7, 0.0),.4);
  float cross = opScale2(q-vec3( 0.0,-0.7, 0.0),0.1);
    vec2 res = opU( vec2( sdPlane(pos), 1.0 ),
	                vec2(cross , 2.0 ) );
   res = opU(res,vec2(menger,2.0));
...
float spec = pow(clamp(dot(ref, normalize(vec3(1,1,1))), 0.0, 1.0), sc3_lightscale); 
  vec3 lights = (spec3*vec3(1,0,0)) + (spec2*plasma(rd.xy))+ (spec*plasma(ref.xy)) + (spec1*plasma(pos.xy)); 
      if(m == 2.)
      {
        ...
        if( m == 1. )
        {
            ...
            col =diff2+rf+tex2  +f+ diff3*vec3(.1,.3,.5)*color2.rgb + spec*plasma(rd.xy)  +lights; 
            col /= res.x *0.74;
       ...
       float occ = calcAO( pos, nor );
		...
		col = col*brdf;
		  ...
		  float sh = clamp( pow(2.4/length(pos - ro), 2.0), 0.0, 1.0);
    vec3 light = normalize(vec3(1.0,0.9,0.3));
    float shf = dot(rd,light);
    float sh1 = pow( max( shf,.9), 1.1);
    vec3 shcol = mix(vec3(.1,.3,.5), vec3(.1,.3,.5), sh1);
    col.rgb = mix(shcol, col.rgb, sh); // add fog    
...
void main( void ) 
{
  vec2 p = 2.0 * gl_FragCoord.xy / resolution - 1.0;
	...
	// camera	
	vec3 ro = vec3( -0.2+3.2*cos(0.1*time + 6.0), 1.0 +.6, 0.6 + 2.*sin(0.5*time + 6.0) );

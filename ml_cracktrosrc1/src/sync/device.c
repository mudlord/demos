...
{
	int i;
	BUF *fp = bufopen(data, len);
	if (!fp)
		return -1;

	bufread(&t->num_keys, sizeof(size_t), 1, fp);
	t->keys = malloc(sizeof(struct track_key) * t->num_keys);
	if (!t->keys)
		return -1;
	...
	for (i = 0; i < (int)t->num_keys; ++i) {
		struct track_key *key = t->keys + i;
		char type;
		bufread(&key->row, sizeof(int), 1, fp);
		bufread(&key->value, sizeof(float), 1, fp);
		bufread(&type, sizeof(char), 1, fp);
		key->type = (enum key_type)type;
	}

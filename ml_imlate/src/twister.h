...
{
	int comp;
	unsigned char* data =LoadImageMemory(logo,logo_len,&twistwidth,&twistheight);
	twistdata = (unsigned char*)malloc(twistwidth*twistheight*4);
	logo_data = (unsigned char*)malloc(twistwidth*twistheight*3);

	for (int i=0;i<twistwidth*twistheight*3;i+=3)
	{
		static int  j=0;
		logo_data[i]=data[j];
		logo_data[i+1]=data[j+1];
		logo_data[i+2]=data[j+2];
		j+=4;
	}
	...
	twistspr.xsize = twistwidth+40;
	twistspr.ysize = twistheight+40;
	twistspr.x = 370;
	twistspr.y = 50;
